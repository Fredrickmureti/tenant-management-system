-- Fix the payments trigger function to handle proper calculations
CREATE OR REPLACE FUNCTION public.payments_sync_billing_paid_amount()
RETURNS TRIGGER AS $$
BEGIN
  IF TG_OP = 'INSERT' THEN
    -- Recalculate paid_amount and current_balance for the billing cycle
    ...
    SET 
      ...
      current_balance = COALESCE(bc.bill_amount, 0) + COALESCE(bc.previous_balance, 0) - COALESCE((
        SELECT SUM(amount) 
        FROM public.payments 
        WHERE billing_cycle_id = NEW.billing_cycle_id
      ), 0),
...
-- Recalculate all existing billing cycle balances to fix data
UPDATE public.billing_cycles bc
SET 
  paid_amount = COALESCE((
    SELECT SUM(amount) 
